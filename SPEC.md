...
`crop_info` 傳的是**相對比例（0-1）加上來源的像素尺寸**，不是像素座標。因為
實際使用時兩邊來源常常不同（例如 1920x1080 的灰模影片，搭配另外算圖產生的
1024x848 上色首幀），直接複製像素數字會框到完全不同的區域。

但**只把 x/y/寬/高各自換算成比例也不夠**：兩邊長寬比不同時（1.778 vs 1.208），
同一組比例會變成不同形狀的框——1248x1080 的橫框會變成 666x848 的直框，形狀
跟位置全跑掉。而且對 i2v 模型來說，影片跟首幀的裁切結果**必須是同一個長寬比**
才配得起來。

所以換算規則是（`map_crop_rect()` / `mapRectAcross()`，前後端各實作一份、行為
一致）：

1. 保持框自己的長寬比不變。
2. 框心放到對應的相對位置。
3. 大小取「照寬度比例」與「照高度比例」兩種算法中，**放得下的最大者**——這樣
   使用者原本貼齊哪一邊就會保留哪一邊：切滿高度的框過去還是滿高度，切滿寬度
   的還是滿寬度。兩種都放不下時再等比縮到放得下。
4. 最後把框推回邊界內（平移，不改變大小）。

例：1920x1080 切 (317, 0, 1248, 1080) → 1024x848 得到 (12, 0, 980, 848)，
長寬比 1.1556 → 1.1557。

接收端的 x/y/width/height 會變成唯讀，並即時鏡射上游那顆節點的框。拔線後恢復
可編輯，並保留拔線當下的數值。

注意這是**幾何換算，不是內容對位**：如果兩張素材本身構圖就不同（物件在畫面中
的大小/位置不一樣），換算後仍需要人工微調。可用內建 `Image Compare` 或
`Image Blend`（difference 模式）檢查。
